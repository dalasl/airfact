...
| 模块 | 对应论文章节 | 功能 |
|------|-------------|------|
| **多模态用户数据特征画像构建** | 第 2 章 | U-Net 去噪 + LayoutLMv3 多模态编码，经双分支 CrossAttention 融合为 896 维特征，通过信息增益加权 K-means（IGW-Kmeans）在内容语义、行为模式、权限上下文三维空间构建动态画像 |
...
> **目标**: 复现论文第五章全部实验结果（表 5.4 ~ 表 5.15, 图 5.1 ~ 图 5.6）。
> **硬件**: 1 台 GPU 服务器 + 3 台终端虚拟机（见 3.1 节硬件配置）。
