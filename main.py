...
def cmd_serve_api(args):
    """Start the API server for client-server deployment.

    Deployment:
        Server (GPU):  python main.py serve-api --host 0.0.0.0 --port 8900
        Agent  (VM):   python main.py agent --server http://<ip>:8900 --watch C:/Users/bob/Documents
    """
    import numpy as np
    ...
    log.info("=" * 60)
    log.info("DLP Detection System — API Server (C/S Mode)")
    log.info("=" * 60)

    # ── Init DLPServer ──
    from src.server import DLPServer
    log.info("[1/3] Loading config: %s", args.config)
    server = DLPServer.from_config(args.config, device=args.device)
    ...
    log.info("[2/3] Three-layer services initialized")

    # ── Create FastAPI app ──
    from src.api_server import create_app
    app = create_app(server, api_token=args.api_token)
    log.info("[3/3] API server starting on %s:%d", args.host, args.port)

    log.info("-" * 60)
    log.info("  Endpoints:")
    log.info("    POST /api/v1/agents/register   — Agent registration")
    log.info("    POST /api/v1/agents/heartbeat   — Agent heartbeat")
    log.info("    POST /api/v1/events             — File event report")
    log.info("    GET  /api/v1/rules/{agent_id}   — Pull pending rules")
    log.info("    WS   /api/v1/ws/{agent_id}      — WebSocket channel")
    log.info("    GET  /api/v1/status              — System status")
    log.info("    GET  /api/v1/agents              — List agents")
...
def cmd_agent(args):
    """Start a terminal agent that connects to a remote API server.

    The agent monitors local files and reports events to the server.
    VQL rules generated by the server are pulled and executed locally.
    """
    from src.terminal_agent import TerminalAgent
    from src.api_client import RemoteClient
    import platform

    agent_id = args.agent_id or f"agent_{platform.node()}"

    log.info("=" * 60)
    log.info("DLP Detection System — Terminal Agent (C/S Mode)")
    log.info("=" * 60)

    # ── Init remote client ──
    log.info("[1/3] Connecting to server: %s", args.server)
    client = RemoteClient(
    ...
    if not client.register(
        ...
        log.error("Failed to register with server. Exiting.")
        sys.exit(1)

    # ── Init terminal agent ──
    log.info("[2/3] Starting terminal agent...")
    agent = TerminalAgent(
        user_id=args.user,
        watch_dirs=[args.watch],
        poll_interval=args.interval,
    ...
    # Wire: agent detects file → client reports to server
    agent.set_event_callback(client.report_event)

    # Wire: server pushes rule → agent loads into VQL executor
    client.set_rule_callback(agent.vql_executor.load_rule)

    # Start heartbeat (also polls rules)
    client.start_heartbeat()

    # Optional: WebSocket for real-time rule push
    if args.ws:
        client.connect_ws()

    agent.start()

    log.info("[3/3] Agent running")
    log.info("-" * 60)
    log.info("  Agent ID   : %s", agent_id)
    log.info("  User ID    : %s", args.user)
    log.info("  Server     : %s", args.server)
    log.info("  Watch dir  : %s", args.watch)
    log.info("  Interval   : %ds", args.interval)
    log.info("  WebSocket  : %s", "enabled" if args.ws else "disabled (HTTP polling)")
    log.info("-" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
examples:
  python main.py serve                               # start detection server (single-process)
  python main.py serve --watch C:/Users/bob/Documents # with file monitoring
  python main.py serve --device cpu                   # CPU-only mode

  # Client-Server deployment (C/S mode):
  python main.py serve-api --port 8900                # server side (GPU)
  python main.py agent --server http://ip:8900 --watch ./data  # agent side (VM)
  ...
  python main.py report                                # view experiment results
    ...
    # --- serve (primary, single-process) ---
    p_serve = sub.add_parser("serve", help="Start the full detection system (single-process)")
    ...
    # --- serve-api (C/S mode: server side) ---
    p_api = sub.add_parser("serve-api", help="Start API server for C/S deployment")
    p_api.add_argument("--config", default="configs/default_config.yaml")
    p_api.add_argument("--device", default="cuda", choices=["cuda", "cpu"])
    p_api.add_argument("--host", default="0.0.0.0", help="API listen address")
    p_api.add_argument("--port", type=int, default=8900, help="API listen port")
    p_api.add_argument("--api-token", default="", help="API authentication token")

    # --- agent (C/S mode: client side) ---
    p_agent = sub.add_parser("agent", help="Start terminal agent connecting to remote server")
    p_agent.add_argument("--server", required=True, help="Server URL (e.g. http://192.168.1.100:8900)")
    p_agent.add_argument("--agent-id", default=None, help="Agent ID (default: hostname)")
    p_agent.add_argument("--user", default="default_user", help="User ID for this agent")
    p_agent.add_argument("--watch", required=True, help="Directory to monitor")
    p_agent.add_argument("--interval", type=int, default=5, help="Poll interval (sec)")
    p_agent.add_argument("--api-token", default="", help="API authentication token")
    p_agent.add_argument("--ws", action="store_true", help="Use WebSocket for rule push")
